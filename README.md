...
Перед разработкой основной части задания следует ознакомиться с описанием признаков и меток датасета, скачав файл по [ссылке](https://www.google.com/url?q=https%3A%2F%2Fstorage.yandexcloud.net%2Facademy.ai%2Fpractica%2Fparkinsons.names). Получим объекты и метки из датафрейма. Объектами являются все столбцы, **кроме** "**status**", а метки - те, что находятся в столбце **‘status’**.
...
Теперь разделим набор данных на обучающий и тестовый наборы, сохранив 20% данных для **тестирования**, что является стандартной практикой:
